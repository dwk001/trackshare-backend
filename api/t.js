          ...
          <div class="container">
            ...
            <div class="providers">
              ${track.providers.map(provider => {
                const escapedDeepLink = (provider.deepLinkHint || '').replace(/'/g, "\\'");
                const escapedWebUrl = (provider.webUrl || '').replace(/'/g, "\\'");
                const isDirect = provider.isDirect || false;
                
                return `
                  <button class="provider-btn ${provider.name} ${isDirect ? 'direct' : 'search'}" onclick="openProvider('${escapedDeepLink}', '${escapedWebUrl}', ${isDirect})">
                    ${
                    ...
                    Play on ${provider.displayName}${isDirect ? ' (Direct)' : ' (Search)'}
                  </button>
                `;
              }).join('')}
            </div>
            ...
            <script>
              function openProvider(nativeUrl, webUrl, isDirect) {
                console.log("Opening provider:", nativeUrl, "Direct:", isDirect);
                
                if (isDirect) {
                  // For direct URLs, try native app first, then web
                  ...
                  // For search URLs, go directly to web
