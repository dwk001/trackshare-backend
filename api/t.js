        ...
        <body>
          <div class="container">
            ...
            <div class="providers">
              ${track.providers.map(provider => {
                if (provider.name === 'spotify') {
                  return `
                    <button class="provider-btn ${provider.name}" onclick="openProvider('${provider.name}', '${encodeURIComponent(provider.deepLink)}', '${encodeURIComponent(provider.webFallback)}')">
            ...
            <script>
              ...
              function buildIntent(webUrl, pkg) {
                const trimmed = webUrl.replace(/^https?:\/\//, '');
                return 'intent://' + trimmed + '#Intent;scheme=https;package=' + pkg + ';S.browser_fallback_url=' + encodeURIComponent(webUrl) + ';end';
              }
              function openProvider(provider, nativeUrlEnc, webUrlEnc) {
                const btn = event.currentTarget || event.target;
                btn.disabled = true;
                btn.style.opacity = '0.7';
                
                const nativeUrl = decodeURIComponent(nativeUrlEnc);
                const webUrl = decodeURIComponent(webUrlEnc);
                let finalNative = nativeUrl;
                
                if (provider === 'apple_music') {
                  if (isAndroid()) {
                    finalNative = buildIntent(webUrl, 'com.apple.android.music');
                  } else if (isIOS()) {
                    finalNative = webUrl; // universal link handoff
                  }
                } else if (provider === 'youtube_music') {
                  if (isAndroid()) {
                    finalNative = buildIntent(webUrl, 'com.google.android.apps.youtube.music');
                  } else if (isIOS()) {
                    finalNative = webUrl; // universal link handoff
                  }
                } else if (provider === 'spotify') {
                  // keep spotify://track or spotify://search as provided
                }
                ...
                const onBlur = () => { didBlur = true; cleanup(); };
                const onVis = () => { if (document.visibilityState !== 'visible') { didBlur = true; cleanup(); } };
                window.addEventListener('blur', onBlur);
                document.addEventListener('visibilitychange', onVis);
                
                // Try native app first
                window.location.href = finalNative;
                
                // Fallback to web after ~900ms if app did not open
                setTimeout(() => {
                  if (!didBlur) {
