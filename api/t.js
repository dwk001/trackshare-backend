      ...
      res.send(`<!DOCTYPE html>
        ...
        <body>
          <div class="container">
            ...
            <div class="providers">
              ${track.providers.map((provider, index) => {
                const escapedWebUrl = (provider.webUrl || '').replace(/'/g, "\\'");
                return `<a href="${provider.webUrl}" 
                   onclick="openMusic('${escapedWebUrl}'); return false;"
                   class="provider-btn ${provider.name}">
                  ${
          ...
          <script>
            function openMusic(webUrl) {
              console.log("Opening:", webUrl);
              window.location.href = webUrl;
              
              setTimeout(function() {
                if (document.hasFocus()) {
                  console.log("App did not open, staying on page");
                }
              }, 1500);
            }
          </script>
