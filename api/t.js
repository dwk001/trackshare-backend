          ...
          <div class="container">
            ...
            <div class="providers">
              ${track.providers.map(provider => `
                <button class="provider-btn ${provider.name}" onclick="openProvider('${provider.name}', '${encodeURIComponent(provider.deepLinkHint)}', '${encodeURIComponent(provider.webUrl)}')">
                  ${
            ...
            <script>
              ...
              function buildIntent(url, pkg) {
                ...
                return 'intent://' + trimmed + '#Intent;scheme=https;package=' + pkg + ';S.browser_fallback_url=' + encodeURIComponent(url) + ';end';
              }
              function openProvider(provider, hintUrlEnc, webUrlEnc) {
                const btn = event.currentTarget || event.target;
                btn.disabled = true;
                btn.style.opacity = '0.7';
                
                const hintUrl = decodeURIComponent(hintUrlEnc);
                const webUrl = decodeURIComponent(webUrlEnc);
                let nativeUrl = hintUrl;
                
                if (isAndroid()) {
                    if (provider === 'apple_music') {
                        nativeUrl = buildIntent(hintUrl, 'com.apple.android.music');
                    } else if (provider === 'youtube_music') {
                        nativeUrl = buildIntent(hintUrl, 'com.google.android.apps.youtube.music');
                    }
                }
